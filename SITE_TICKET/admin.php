...
try {
    $pdo = new PDO('mysql:host=localhost;dbname=ticket_233', 'root', 'root'); // Vérifie tes identifiants
...
// -------------------------
// Récupération des utilisateurs avec leurs rôles et permissions
// -------------------------
try {
    ...
    // Organisation des résultats par utilisateur en stockant aussi l'ID
    $users = [];
                        ...
                        <tbody>
                            <?php foreach ($tickets as $ticket): ?>
                                <tr class="bg-gray-800 hover:bg-gray-700 border-b">
                                    ...
                                    <td class="px-4 py-2">
                                        <a href="ticket_view.php?id=<?= htmlspecialchars($ticket['Id']) ?>" class="text-blue-400 hover:underline">Voir</a>
                                        <!-- Boutons supplémentaires pour modifier/supprimer peuvent être ajoutés ici -->
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <?php if (count($users) > 0): ?>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <?php foreach ($users as $user): ?>
                        <?php
                        // Détermine la classe badge en fonction du rôle (mêmes couleurs que profil.php)
                        ...
                        <div class="bg-gray-700 p-4 rounded-lg shadow">
                            ...
                            <div class="mt-4">
                                <a href="edit_user.php?id=<?= htmlspecialchars($user['user_id']) ?>" class="px-3 py-1 bg-green-500 rounded hover:bg-green-600 text-sm">Modifier</a>
                                <a href="delete_user.php?id=<?= htmlspecialchars($user['user_id']) ?>" class="px-3 py-1 bg-red-500 rounded hover:bg-red-600 text-sm">Supprimer</a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
